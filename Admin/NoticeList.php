<div id="content">
<ul class="breadcrumb">
    <li><a href="Notice.php" class="glyphicons home"><i></i>Danh sách thông báo</a></li>
</ul>
<div class="heading-buttons">
    <h3 class="glyphicons display"><i></i>Quản lý thông báo</h3>
    <div class="buttons pull-right">
        <a class="btn btn-primary btn-icon glyphicons circle_plus" href="NoticeAdd.php"><i></i>Add new</a>
    </div>
...
<div class="widget-head">
    <h4 class="heading">Danh sách thông báo</h4>
</div>
        ...
        if ($selectArray == null) {
            ...
            foreach ($selectArray as $selectArrayItem) {
                ...
                <tr class="gradeA odd">
                    ...
                    <td class=""><?php echo $selectArrayItem->no_title; ?></td>
                    <td class=" "><?php echo $selectArrayItem->no_date; ?></td>
                    <td class=" "><?php echo $selectArrayItem->no_author; ?></td>
                    <td class=" center " id="ajax<?php echo $selectArrayItem->no_id; ?>">
                        <a onclick="state(<?php echo $selectArrayItem->no_id; ?>,<?php echo $selectArrayItem->no_state; ?>)">
                            <?php if ($selectArrayItem->no_state == 1) {
                    ...
                    <td class="center  sorting_1"><a
                            href="NoticeUpdate.php?id=<?php echo $selectArrayItem->no_id; ?>">Edit</a>
                    </td>
                    <td class="center "><a
                            href="NoticeDelAction.php?id=<?php echo $selectArrayItem->no_id; ?>"
                            class="Del">Del</a></td>
                </tr>
            <?php
            }
        ...
        <div style="margin: 0;" class="pagination pagination-small pull-right">
            <ul>
                ...
                if ($page > 1) {
                    ...
                    if ($current != 1) {
                        ?>
                        <li><a href='Notice.php?rows=<?php echo $display ?>&start=<?php echo $prev ?>'>Previous</a>
                        </li>
                    <?php
                    }
                    // Hien thi so link
                    for ($i = 1; $i <= $page; $i++) {
                        if ($current != $i) {
                            ?>
                            <li>
                                <a href='Notice.php?rows=<?php echo $display ?>&start=<?php echo($display * ($i - 1)) ?>'><?php echo $i ?></a>
                            </li>
                        <?php
                        } else {
                            ?>
                            <li class="active"><a
                                    href='Notice.php?rows=<?php echo $display ?>&start=<?php echo($display * ($i - 1)) ?>'><?php echo $i ?></a>
                            </li>
                        <?php
                        }
                    }
                    ...
                    if ($current != $page) {
                        ?>
                        <li><a href='Notice.php?rows=<?php echo $display;?>&start=<?php echo $next;?>'>Next</a></li>
                    <?php
                    }
                }
                ?>
...
<script>
    ...
    function state(id, no_state) {
        ...
        $.ajax({
            type: "POST",
            url: "NoticeAjaxState.php?id=" + id + "&ne_state=" + no_state,
...
<script>
    function Pagination() {
        location.href = "Notice.php?rows=" + $('#pagination').val();
    }
</script>
